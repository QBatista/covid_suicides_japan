...
def unemp_suicide_plot(df_suicide, df_unemp, data_type='total'):
    ...
    x_vals = df_unemp.index.intersection(df_suicide.index)
    ...
    fig.update_xaxes(title_text=data_type_cap + " Unemployment Rate",
                     row=1, col=2)

    # Update yaxis properties
    fig.update_yaxes(title_text=data_type_cap + " Number of Suicides",
                     row=1, col=2)
...
def fig_3_to_8(dfs, params, output_path):
    ...
    fig = unemp_suicide_plot(df_suicide_monthly[start_date:],
                             df_unemp_monthly[start_date:],
                             data_type='total')
    ...
    fig = unemp_suicide_plot(df_suicide_annual[start_date:],
                             df_unemp_annual[start_date:],
                             data_type='total')
    fig.write_image(path, format='pdf')
